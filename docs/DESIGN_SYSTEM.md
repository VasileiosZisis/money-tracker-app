...
- URL-backed inline editors and disclosures preserve the current viewport when opening or closing. Use no-scroll navigation for View/Edit, Close/Cancel, Manage, and equivalent in-page controls; normal page-to-page links retain standard scroll behavior.
...
- Dashboard composition uses five distinct responsive rows: Total Balance with Monthly Snapshot (`md` two-column), Spending by category at full width, the five forecast metrics, Needs Attention at full width, and Transactions & Plans. Forecast metrics use one column below `1280px` and two equal columns from `1280px`, ordered as Forecast remaining spend | Safe to spend, Daily safe spend | Weekly safe spend, and Spending pace | empty. In Transactions & Plans, Recent Transactions occupies one column and a unified Planned Items card occupies two columns from `1280px`; below that breakpoint, both cards use separate full-width rows with Planned Items below Recent Transactions.
- Forecast remaining spend, Safe to spend, Daily safe spend, Weekly safe spend, and Spending pace use the same compact label/value block with the icon vertically centered against that block and no descriptive helper below the amount. For the current selected month, Forecast remaining spend places a non-wrapping semantic confidence badge directly to the right of its label: success/green for High, warning/orange for Medium, and destructive/red for Low. Past and future selected months omit the confidence badge. Spending pace uses warning/orange above usual, success/green below usual, accent on pace, and outline when unavailable; above-usual historical-baseline context appears as a Needs Attention item instead of metric-card helper text.
- Use a `Planning & Forecast` section heading above the five forecast metric cards and Needs Attention to distinguish planning-oriented information from Total Balance and Monthly Snapshot.
...
- The Needs Attention card uses a title-only header without supporting copy or a numeric count badge, and uses compact `p-3` content padding. Its signals use one column on small screens and two columns from `md`, allowing the six-item limit to form three rows; the empty state spans the full card width.
