...
function formatCategoryLabel(category: PlannedIncomeFormCategory) {
  return `${category.name}${category.isArchived ? " (archived)" : ""}`;
}

export function PlannedIncomeFormFields({
  idPrefix,
  currency,
  categories,
  defaultValues,
  ...
  idPrefix: string;
  currency: string;
  categories: PlannedIncomeFormCategory[];
      ...
      <div className={cn("grid gap-4", !singleColumn && "md:grid-cols-2")}>
        <FormField htmlFor={`${idPrefix}-name`} label="Name">
          <Input
            ...
            placeholder="Salary, freelance, support..."
            maxLength={120}
            required
          />
        </FormField>
      ...
      <div className={cn("grid gap-4", !singleColumn && "md:grid-cols-2")}>
        <FormField htmlFor={`${idPrefix}-expected-day`} label="Expected day of month">
          <Input
            ...
            min={1}
            max={28}
            defaultValue={defaultValues.expectedDayOfMonth}
            required
          />
        </FormField>
